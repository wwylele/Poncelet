...
We call $(V, E)$ \emph{singular} if $\mathrm{next}(V, E) = (V, E)$. This is important because if 
the two circles are tangent to each other, and a vertex lands exactly
on it, applying $\mathrm{next}$ will resulting in a degenerate polygon where all vertices coincide.
Obviously this doesn't tell how other vertices behaves, so we will need to exclude it from the theorem.
We characterize singular point with the following
\begin{lemma}
...
\end{lemma}
\begin{proof}
    The first part is directly from the property of $\mathrm{rPoint}$ and $\mathrm{rChord}$. One can
    then solve the system of equations for $V$ and $E$. 
\end{proof}

We now formalize the statement of a version of Poncelet's closure theorem
\begin{theorem}
    Given a two circles configuration in field $K$, if for a natural number $n$, and some non-singular
...
The curve $E$ is not always an elliptic curve because it can be singular. Precisely speaking,
under the constraints $u \ne 0$ and $r \ne 0$, this curve is singular if and only if 
$(u + r)^2 = 1$ or $(u - r)^2 = 1$, meaning that the two circles are tangent to each other. In
...
With the map $f$, we can make connection between the elliptic curve $E$ and the two circles
